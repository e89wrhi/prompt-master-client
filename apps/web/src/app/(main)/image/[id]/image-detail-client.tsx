...
export function ImageDetailClient({
  ...
  return (
    <div className="flex w-full max-w-[1400px] mx-auto py-8 px-4 sm:px-6 gap-10">
      ...
      <div className="w-64 hidden lg:flex flex-col gap-6 border-r pr-6 border-neutral-200 dark:border-neutral-800 sticky top-8 h-[calc(100vh-4rem)]">
        <div className="bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md py-4 z-10 border-b border-transparent">
          <Link
            href="/image"
        ...
        <div className="flex-1 overflow-y-auto pb-20 no-scrollbar pr-2">
          <div className="flex flex-col gap-5">
            {allPrompts.map((p) => {
              const isActive = p.id === prompt.id;
              const hasPDimensions = p.width && p.height;
              return (
                <Link
                  key={p.id}
                  href={`/image/${p.id}`}
                ...
                >
                  <div 
                    className="relative w-full bg-neutral-100 dark:bg-neutral-800"
                    style={{ aspectRatio: hasPDimensions ? `${p.width} / ${p.height}` : '4/3' }}
                  >
                    <Image
                      src={p.image || '/sport.png'}
                      alt={p.title}
                      fill
      ...
      <div className="flex-1 flex flex-col gap-8 max-w-4xl min-w-0">
        {/* Large Image Header */}
        <div 
          className="w-full relative rounded-3xl overflow-hidden shadow-2xl ring-1 ring-black/5 dark:ring-white/10 group"
          style={{ 
            aspectRatio: prompt.width && prompt.height ? `${prompt.width} / ${prompt.height}` : '21/9' 
          }}
        >
          <Image
