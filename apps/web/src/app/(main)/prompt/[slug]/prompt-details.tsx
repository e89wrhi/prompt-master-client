...
export function PromptDetails({ prompt }: PromptDetailsProps) {
  ...
  function highlightToHTML(text: string) {
    return text
      .replaceAll(
        '@@HL_START@@',
        '<span class="bg-amber-100 dark:bg-amber-900/40 text-amber-900 dark:text-amber-100 px-1.5 py-0.5 rounded font-bold border border-amber-200 dark:border-amber-800">'
      )
      .replaceAll('@@HL_END@@', '</span>');
  }

  const openInAI = (model: 'gpt' | 'gemini' | 'grok') => {
    const markdownContent = `# ${editedTitle}\n\n${editedDescription}\n\n## Template\n${renderedTemplate}`;
    const promptText = `You are reading documentation from my website. Explain this page and answer questions about it.\n\nMarkdown:\n${markdownContent}`;

    let url = '';
    if (model === 'gpt') {
      url = `https://chat.openai.com/?prompt=${encodeURIComponent(promptText)}`;
    } else if (model === 'gemini') {
      url = `https://gemini.google.com/app?q=${encodeURIComponent(promptText)}`;
    } else if (model === 'grok') {
      url = `https://x.com/i/grok?text=${encodeURIComponent(promptText)}`;
    }
        ...
        <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
                  ...
                  <div className="mt-8 flex flex-wrap gap-4 items-center">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          onClick={() => openInAI('gpt')}
                          className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 transition-all group"
                        >
                          <div className="relative w-5 h-5 overflow-hidden rounded-sm grayscale group-hover:grayscale-0 transition-all scale-110">
                            <Image
                              src="/models/gpt.png"
                              alt="GPT"
                              fill
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 transition-all group"
                        >
                          <div className="relative w-5 h-5 overflow-hidden rounded-md grayscale group-hover:grayscale-0 transition-all scale-110">
                            <Image
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              ...
              onClick={() => handleCopy(renderedTemplate)}
            >
              {copiedMap['main'] ? (
            ...
            <Button
              size="lg"
              className="rounded-full bg-amber-500 hover:bg-amber-600 text-black border-none"
                ...
                <div className="bg-neutral-50 dark:bg-neutral-900 px-6 py-4 flex justify-between items-center border-b">
                  ...
                  {isEditing && (
                    <Button
                      ...
                      onClick={() =>
                        setEditedExamples(
                          editedExamples.filter((_, i) => i !== idx)
                        )
                      }
                    >
                      <X className="h-4 w-4" />
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {!isEditing && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleCopy(ex.input, `in-${ex.id}`)}
                        >
                          {copiedMap[`in-${ex.id}`] ? (
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {!isEditing && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleCopy(ex.output, `out-${ex.id}`)}
                        >
                          {copiedMap[`out-${ex.id}`] ? (
                            <Check className="h-4 w-4" />
                          ) : (
